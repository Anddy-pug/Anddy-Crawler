...
class WebCrawler_openproject:
    def __init__(
        ...
        try:
            ...
            self.driver = driver = webdriver.Chrome(service=Service(r'G:\driver\129.0.6668.70\chromedriver-win32\chromedriver.exe'), options=chrome_options)
    ...
    def login(self):    # openproject
        ...
        try:
            ...
            self.driver.find_element(By.ID, "username-pulldown").send_keys(self.target_username)
            self.driver.find_element(By.ID, "password-pulldown").send_keys(self.target_password)
            self.driver.find_element(By.ID, "login-pulldown").click()
    ...
    def is_valid_url(self, url: str) -> bool:
        ...
        return (
            parsed_url.scheme in ("http", "https") and
            'work_packages' in parsed_url.path and  # Ensure the URL contains 'work_packages'
            not url.endswith(self.not_url)
    )
