...
def get_image_description_from_server(image_url):
    # URL of your Flask server endpoint
    embedding_server_url = "http://localhost:6666/caption"
    
    try:
        # Send a POST request with the image URL
        response = requests.post(embedding_server_url, json={"image_url": image_url})
        
        # Check if the response is successful
        response.raise_for_status()
        
        # Parse the JSON response and return the caption
        response_data = response.json()
        return response_data.get('caption', 'No caption found')  # Get the caption from the response

    except requests.exceptions.RequestException as e:
        print(f"Error contacting captioning server: {e}")
        return None
...
def main_indexing(mypath):
    ...
    # Base URL for Apache
    base_url = "http://192.168.140.236/devops"
    ...
    for file in onlyfiles:
        if file.lower().endswith(supported_formats):
            ...
            print("indexing " + file)
            print("url :" + file_url)
            description = get_image_description_from_server(file_url)
            print(description)
            embedding = get_embeddings_from_server(description)
            ...
            es.index(index="primary-image-description", document=doc_body)
