...
class ReactSpiderSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.upwork.com/nx/search/jobs/"]
    max_pages = 100 
    ...
    def parse(self, response):
        ...
        for job_item in response.css('article[data-test="JobTile"]'):
            # Extract the URL from the anchor tag inside the job title
            job_url = job_item.css('h2.job-tile-title a::attr(href)').get()
            if job_url:
        ...
        if self.page_number < self.max_pages:
            self.page_number += 1
            next_page_url = f"https://www.upwork.com/nx/search/jobs/?page={self.page_number}"  # Increment page number in URL
