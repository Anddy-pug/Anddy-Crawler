...
class WebCrawler_nextcloud:
    def __init__(
        ...
        try:
            ...
            # self.driver = driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=chrome_options)
    ...
    def login(self):    # openproject
        ...
        try:
            ...
            self.driver.find_element(By.ID, "user").send_keys(self.target_username)
            self.driver.find_element(By.ID, "password").send_keys(self.target_password)


            login_button = self.driver.find_element(By.CSS_SELECTOR, "button.button-vue.button-vue--icon-and-text.button-vue--vue-primary.button-vue--wide")
            login_button.click()
    ...
    def is_valid_url(self, url: str) -> bool:
        ...
        return (
            parsed_url.scheme in ("http", "https") and
            'files' in parsed_url.path and  # Ensure the URL contains 'work_packages'
            not url.endswith(self.not_url)
    )
    ...
    def crawl_page(self, url: str) -> List[str]:
        ...
        try:
            ...
            if self.is_valid_url(url):
                ...
                results = self.get_file_items(page_source)
                soup = BeautifulSoup(self.driver.page_source, "html.parser")

                favicon_url = self.extract_favicon(soup)
                for result in results:
                    # print(f"File Name: {result['file_name']}, Image URL: {result['image_url']}")
                    print(f"File Name: {result['file_name']}, Image URL: {favicon_url}")
                    file_url = url + "/" + result['file_name']
                    file_url.replace("//", "/")
                # Save the HTML content of the page
                    with self.visited_urls_lock:
                        # self.send_to_elasticsearch(file_url, result['file_name'], "This is a nextcloud file", result['image_url'])
                        self.send_to_elasticsearch(file_url, result['file_name'], "This is a nextcloud file", favicon_url)
                        self.visited_urls.add(file_url)
    ...
    def get_file_items(self, page_source):
        soup = BeautifulSoup(page_source, 'html.parser')

        # Find all <td> elements that contain the class 'files-list__row-name'
...
if __name__ == "__main__":
    ...
    #     index_name="product_web_nextcloud",
    #     login_url="https://192.168.140.254:33001/login",
    #     target_username="pug0620",
    #     target_password="pug0620",
    #     username_field_id="username",
    #     password_field_id="password",
    #     submit_button_id="login",
    #     base_url="https://192.168.140.254:33001",
    #     not_url="logout"
    # )